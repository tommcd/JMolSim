...
PROGRAM SIMBOX
      ...
      character(len=132):: infile,carg,fname
      integer:: io_temp,io_energy,io_config,io_config_final
...
!
      ...
      open(unit=io_config,file=trim(infile(1:len)))
!
...
!
      ...
      if (stat /= 0) then
         ...
         open(unit=io_restart_xyz,file=trim(infile(1:len)))
         call get_command_argument (6, infile, len, stat)
         if (stat /= 0) then
            print *,'restart velocity file missing'
            stop
         else
            open(unit=io_restart_v,file=trim(infile(1:len)))
         end if
      end if
...
!print *,'msd_gas%iout = ',msd_gas%iout
!print *,'vcf_gas%iout = ',vcf_gas%iout
!inquire(file=trim('msd_GAS.out'),iostat=ios,exist= lexist,opened= lopen,number=ionum)
!print *,'msd_GAS.out',msd_gas%iout
!print *, ios,lexist,lopen,ionum
!inquire(file=trim('vcf_GAS.out'),iostat=ios,exist= lexist,opened= lopen,number=ionum)
!print *,'vcf_GAS.out',vcf_gas%iout
!print *,ios,lexist,lopen,ionum
!
!inquire(unit=msd_gas%iout,iostat=ios,exist= lexist,opened= lopen,number=ionum,name=fname)
!print *,'msd_GAS.out',msd_gas%iout
!print *, ios,lexist,lopen,ionum,trim(fname)
!inquire(unit=vcf_gas%iout,iostat=ios,exist= lexist,opened= lopen,number=ionum,name=fname)
!print *,'vcf_GAS.out',vcf_gas%iout
!print *,ios,lexist,lopen,ionum,trim(fname)
!
!
!print *,'msd_O2%iout = ',msd_O2%iout
!print *,'vcf_O2%iout = ',vcf_O2%iout
!inquire(file=trim('msd_O2.out'),iostat=ios,exist= lexist,opened= lopen,number=ionum)
!print *,'msd_O2.out',msd_O2%iout
!print *, ios,lexist,lopen,ionum
!inquire(file=trim('vcf_O2.out'),iostat=ios,exist= lexist,opened= lopen,number=ionum)
!print *,'vcf_O2.out',vcf_O2%iout
!print *,ios,lexist,lopen,ionum
!
!inquire(unit=msd_O2%iout,iostat=ios,exist= lexist,opened= lopen,number=ionum,name=fname)
!print *,'msd_O2.out',msd_O2%iout
!print *, ios,lexist,lopen,ionum,trim(fname)
!inquire(unit=vcf_O2%iout,iostat=ios,exist= lexist,opened= lopen,number=ionum,name=fname)
!print *,'vcf_O2.out',vcf_O2%iout
!print *,ios,lexist,lopen,ionum,trim(fname)
!
!
!print *,'msd_N2%iout = ',msd_N2%iout
!print *,'vcf_N2%iout = ',vcf_N2%iout
!inquire(file=trim('msd_N2.out'),iostat=ios,exist= lexist,opened= lopen,number=ionum)
!print *,'msd_N2.out',msd_N2%iout
!print *, ios,lexist,lopen,ionum
!inquire(file=trim('vcf_N2.out'),iostat=ios,exist= lexist,opened= lopen,number=ionum)
!print *,'vcf_N2.out',vcf_N2%iout
!print *,ios,lexist,lopen,ionum
!
!inquire(unit=msd_N2%iout,iostat=ios,exist= lexist,opened= lopen,number=ionum,name=fname)
!print *,'msd_N2.out',msd_N2%iout
!print *, ios,lexist,lopen,ionum,trim(fname)
!inquire(unit=vcf_N2%iout,iostat=ios,exist= lexist,opened= lopen,number=ionum,name=fname)
!print *,'vcf_N2.out',vcf_N2%iout
!print *,ios,lexist,lopen,ionum,trim(fname)
!
!
!print *,'msd_CO2%iout = ',msd_CO2%iout
!print *,'vcf_CO2%iout = ',vcf_CO2%iout
!inquire(file=trim('msd_CO2.out'),iostat=ios,exist= lexist,opened= lopen,number=ionum)
!print *,'msd_CO2.out',msd_CO2%iout
!print *, ios,lexist,lopen,ionum
!inquire(file=trim('vcf_CO2.out'),iostat=ios,exist= lexist,opened= lopen,number=ionum)
!print *,'vcf_CO2.out',vcf_CO2%iout
!print *,ios,lexist,lopen,ionum
!
!inquire(unit=msd_CO2%iout,iostat=ios,exist= lexist,opened= lopen,number=ionum,name=fname)
!print *,'msd_CO2.out',msd_CO2%iout
!print *, ios,lexist,lopen,ionum,trim(fname)
!inquire(unit=vcf_CO2%iout,iostat=ios,exist= lexist,opened= lopen,number=ionum,name=fname)
!print *,'vcf_CO2.out',vcf_CO2%iout
!print *,ios,lexist,lopen,ionum,trim(fname)
!stop
...
!
      ...
      if (istep <= nequib) then
         ...
         do m = 1, n_O2
            do i = 1,2
               Ek_O2 = Ek_O2 + (0.5_wp/Ox_massi(m,i,1))*dot_product(Ox_vxyz(m,i,:),Ox_vxyz(m,i,:))
            end do
         end do
         Ek_N2 = 0.0_wp
         do m = 1, n_N2
            do i = 1,2
               Ek_N2 = Ek_N2 + (0.5_wp/N2_massi(m,i,1))*dot_product(N2_vxyz(m,i,:),N2_vxyz(m,i,:))
            end do
         end do
         Ek_CO2 = 0.0_wp
         do m = 1, n_CO2
            do i = 1,3
               Ek_CO2 = Ek_CO2 + (0.5_wp/CO2_massi(m,i,1))*dot_product(CO2_vxyz(m,i,:),CO2_vxyz(m,i,:))
            end do
         end do
         ...
         if (mod(istep,nsamp) == 0) then
            ...
            write(io_energy,'(8g18.8)') time, EK_tot, energy + repulenergy*0.5_wp, EK_tot + energy + repulenergy*0.5_wp
      ...
      if (istep > nequib) then
         ...
         write(io_energy,'(8g18.8)') time, Ek_tot, energy + repulenergy*0.5_wp,Ek_tot + energy + repulenergy*0.5_wp
         write(io_temp,'(8g18.8)') time, t_kinetic, 2.0_wp*ek/(k_ev*(3*natom)), 2.0_wp*ek_gas/(k_ev*(3*(n_atom_gas*n_gas))), &
             2.0_wp*ek_o2/(k_ev*(3*(n_o2*2))), 2.0_wp*ek_n2/(k_ev*(3*(n_n2*2))),2.0_wp*ek_co2/(k_ev*(3*(n_co2*3)))

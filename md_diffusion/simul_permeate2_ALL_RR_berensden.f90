...
PROGRAM SIMBOX
      ...
      integer,parameter:: number_moves = 1002000
...
!      TKE = 2.0*KE/(3*N)
!      lambda = sqrt(1.0_wp + (dt/tau)*(Tb/TKE-1.0_wp))
!      KE = 0.0
!      vxyz = lambda*vxy
!      KE = sum(dot_product(vxyz(i,:),vxyz(i:)))
...
!         write (17,'(8g18.8)') time, Ek,EkO2,EkN2,EkCO2, Ek+energy+repulenergy*0.5_wp
!         call flush(17)
!         write(*,*) istep,timep(2)-timep(1),timep(3)-timep(2),timep(3)-timep(0)
         if (make_movie) then
            iframe = iframe + 1

...
!
   ...
   real(wp),parameter:: AOO = 0.1169_wp     ! nm
   real(wp),parameter:: ANN = 0.1097_wp     ! nm
   real(wp),parameter:: ACO2 = 0.2324_wp    ! nm
...
CONTAINS
   ...
   SUBROUTINE FORCE_ENERGY_EL(sysfirst,syslast,gasfirst,gaslast,nat_gas, &
      ...
      real(wp):: rgas(3),rr(3),df(3),r2,r1,du,rrc2
      integer:: L,M,n
      UEL = 0.0_wp
      mol_loop: do M = gasfirst,gaslast
         atom_loop: do n = 1,nat_gas
            ...
            solid_loop: do L = sysfirst,syslast
               ...
               call pbc(rr)
               r2 = rr(1)**2 + rr(2)**2 + rr(3)**2
if (r2 >= rcutel2) cycle solid_loop
               r1 = sqrt(r2)
               du = charge(L)*charge_gas(M,n)/r1
rrc2 = (r2/rcutel2)
               df = du*(rr/r2)*(1.0_wp + 2.0_wp*rrc2 - 3.0_wp*rrc2**2)
du = du*(1.0_wp - rrc2)**2
               fxyz(L,1:3) = fxyz(L,1:3) - df
               fxyz_gas(M,n,1:3) = fxyz_gas(M,n,1:3) + df
               Uel = Uel + du
   ...
   SUBROUTINE FORCE_ENERGY_EL_X2_dummy(sysfirst,syslast,gasfirst,gaslast, &
      ...
      real(wp),intent(inout):: fxyz_gas(:,:,:)
      real(wp),intent(out):: UEL
      real(wp):: rgas(3,3),rr(3),df(3),r2,r1,du,rrc2
      integer:: L,M,n
      UEL = 0.0_wp
      mol_loop: do M = gasfirst,gaslast
         rgas(1,:) = rxyz_gas(M,1,:)
...
PROGRAM SIMBOX
      ...
      real(wp)::CL,time,Ek,rad_Ox(3),rad_N2(3),rad_CO2(3),rr(3),ULJEL(40),UEL
      real(wp):: e_bond_O2,e_bond_CO2(40),e_bond_N2,e_angle_CO2
...
print *,i,'e_bond_o2 ',e_bond_o2

call n2_stretching(e_bond_n2)
print *,i,'e_bond_n2 ',e_bond_n2

call CO2_stretching(e_bond_co2)
do i = 1,n_co2
 print *,i,e_bond_co2(i)
end do
...
do i = 1,n_co2
if (any( abs(co2_xyz(i,:,:)) > boxl2 ) ) then
print *,'################ 1'
print *,'i = ',i
print *,co2_xyz(i,1,:)
print *,co2_xyz(i,2,:)
...
print *,'################ 2'
print *,'i = ',i
print *,(co2_xyz(i,1,j),j = 1,3)
print *,(co2_xyz(i,2,j),j = 1,3)
print *,(co2_xyz(i,3,j),j = 1,3)
print *,'istep = ',istep
end if
end do
do i = 1,natom
if (any( abs(rxyz(i,:)) > boxl2 ) ) then
print *,'################ 3'
print *,'i = ',i
print *,(rxyz(i,j),j = 1,3)
print *,'istep = ',istep
exit
end if
end do
ii = 26
write(97,55) time,(co2_xyz(ii,1,j),co2_xyz(ii,2,j),co2_xyz(ii,3,j),j = 1,3)
write(98,55) time,(r_CO2_uc(ii,1,j),r_CO2_uc(ii,2,j),r_CO2_uc(ii,3,j),j = 1,3)
write(99,55) time,(rcm_co2_uc(ii,j),j = 1,3)
55 format(g12.6,3(3x,3f9.3))

...
!
      ...
      do i = 1,nmol
         write(iu,110)'O2 ',(Ox_xyz(i,1,j)*Angstrom,j = 1,3)
         write(iu,110)'O2 ',(Ox_xyz(i,2,j)*Angstrom,j = 1,3)
      end do
      do i = 1,nmol
         write(iu,110)'N2 ',(N2_xyz(i,1,j)*Angstrom,j = 1,3)
         write(iu,110)'N2 ',(N2_xyz(i,2,j)*Angstrom,j = 1,3)
      end do
      do i = 1,nmol
...
!
      ...
      do i = 1,nmol
         write(iu,*) (Ox_vxyz(i,1,j),j = 1,3)
         write(iu,*) (Ox_vxyz(i,2,j),j = 1,3)
      end do
      do i = 1,nmol
         write(iu,*) (N2_vxyz(i,1,j),j = 1,3)
         write(iu,*) (N2_vxyz(i,2,j),j = 1,3)
      end do
      do i = 1,nmol
...
! first check if there is a large overlap
   ...
   FUNCTION overlap(fr,rad,mol_vec1)
      ...
      real(wp):: dr(3),rad(3),fr,mol_vec1(3,3),sigj2
      overlap = .false.
...
!      en = 0.0_wp
      ...
      en(1:n_o2) = en(1:n_o2)*0.5_wp*KOO
      energy = energy + sum(en(1:n_o2))
...
!     Force routine for Keating bond stretching
      ...
      en(1:n_n2) = en(1:n_n2)*0.5_wp*KNN
      energy = energy + sum(en(1:n_n2))
    END SUBROUTINE
...
!         CO2_fxyz(i,3,1:3) = CO2_fxyz(i,3,1:3)-coef*(rl1/coef2-rl1l2*rl2/coef3)
      ...
      en(1:n_CO2) = en(1:n_CO2)*0.5_wp*K_CO2
      energy = energy + sum(en(1:n_CO2))
   END SUBROUTINE
...
PROGRAM SIMBOX
      ...
      USE vcf_mod
      implicit none
      integer,parameter:: number_moves = 1005000
      real(wp),parameter:: dtreal = 1.0_wp
      integer,parameter:: n_short = 4
      integer,parameter:: nequib = 5000
      ...
      real(wp)::CL,time,Ek,EkO2,EkN2,EkCO2,rad_Ox(3),rad_N2(3),rad_CO2(3),rr(3),ULJEL(40),UEL,dt_short
      real(wp):: e_bond_O2(40),e_bond_CO2(40),e_bond_N2(40),e_angle_CO2(40)
      real(wp),allocatable:: massi(:,:),Ox_massi(:,:,:),N2_massi(:,:,:),CO2_massi(:,:,:)
      integer:: j,i,m,ii,ic = 0,natomc,istep,len,stat,nat
...
!
      ...
      call get_command_argument (4, infile, len, stat)
      if (stat /= 0) then
         write (*,*) 'get_command_argument failed: status = ', stat, ' arg = ', 4
         restart = .false.
      else
         restart = .true.
...
N2_fxyz = 0.0_wp
CO2_fxyz = 0.0_wp
call o2_stretching(e_bond_o2)
print *,i,'e_bond_o2 ',sum(e_bond_o2)

call n2_stretching(e_bond_n2)
print *,i,'e_bond_n2 ',sum(e_bond_n2)

call CO2_stretching(e_bond_co2)
do i = 1,n_CO2
 print *,i,e_bond_co2(i),e_angle_co2(i)
end do
...
! only LJ using neighbour list
      ...
      call FORCE_ENERGY_LJ_NLIST(1,n_CO2,3,CO2_atom,CO2_xyz,CO2_fxyz,ULJEL); print*,'ULJ CO2 ',sum(ULJEL)
! Electrostatic
!      call FORCE_ENERGY_EL_X2_dummy(1,natom,1,n_O2,Ox_gas_charge,Ox_xyz,Ox_fxyz,UEL); print*,'Uel O2  ',Uel
!      call FORCE_ENERGY_EL_X2_dummy(1,natom,1,n_n2,n2_gas_charge,n2_xyz,n2_fxyz,UEL); print*,'Uel N2  ',Uel
!      call FORCE_ENERGY_EL(1,natom,1,n_CO2,3,co2_gas_charge,co2_xyz,co2_fxyz,UEL); print*,'Uel CO2 ',Uel
...
do i = 1,5
 print *,'i = ',i
 print '(3(1x,f16.10))',(Ox_fxyz(i,1,j),j = 1,3)
 print '(3(1x,f16.10))',(Ox_fxyz(i,2,j),j = 1,3)
! print '(3(1x,f16.10))',(CO2_fxyz(i,3,j),j=1,3)
end do
...
do i = 1,n_CO2
if (any( abs(ox_xyz(i,:,:)) > boxl2 ) ) then
print *,'################ 1'
print *,'i = ',i
print *,(ox_xyz(i,1,j),j = 1,3)
print *,(ox_xyz(i,2,j),j = 1,3)
!print *,(co2_xyz(i,3,j),j=1,3)
print *,'istep = ',istep
print *,'before gas_vverlet_a'
end if
end do
...
print *
do i = 1,n_CO2
 print '(i3,4(1x,f16.10))',i,e_bond_o2(i),e_bond_n2(i),e_bond_co2(i),e_angle_co2(i)
end do
if (make_movie) then
iframe = iframe + 1
call new_frame_file(imve,'frame',iframe)
...
!     call FORCE_ENERGY_LJ_EL_CO2(1,natom,1,n_CO2,ULJEL)
! only LJ
!      call FORCE_ENERGY_LJ(1,natom,1,n_O2,2,Ox_atom,Ox_xyz,Ox_fxyz,ULJEL)!; print*,'ULJ O2  ',sum(ULJEL)
!      call FORCE_ENERGY_LJ(1,natom,1,n_N2,2,N2_atom,N2_xyz,N2_fxyz,ULJEL)!; print*,'ULJ N2  ',sum(ULJEL)
!      call FORCE_ENERGY_LJ(1,natom,1,n_CO2,3,CO2_atom,CO2_xyz,CO2_fxyz,ULJEL)!; print*,'ULJ CO2 ',sum(ULJEL)
! only LJ using neighbour list
      call FORCE_ENERGY_LJ_NLIST(1,n_O2,2,Ox_atom,Ox_xyz,Ox_fxyz,ULJEL); print*,'ULJ O2  ',sum(ULJEL)
      call FORCE_ENERGY_LJ_NLIST(1,n_N2,2,N2_atom,N2_xyz,N2_fxyz,ULJEL); print*,'ULJ N2  ',sum(ULJEL)
      call FORCE_ENERGY_LJ_NLIST(1,n_CO2,3,CO2_atom,CO2_xyz,CO2_fxyz,ULJEL); print*,'ULJ CO2 ',sum(ULJEL)
...
do i = 1,n_CO2
 ...
 print '(3(1x,f16.10))',(Ox_fxyz(i,1,j),j = 1,3)
 print '(3(1x,f16.10))',(Ox_fxyz(i,2,j),j = 1,3)
! print '(3(1x,f16.10))',(O2_fxyz(i,3,j),j=1,3)
end do
...
!        end do
         ...
         T(2) = Ek*2.0_wp/( 3.0_wp*K_ev*(3*n_CO2 + 2*n_o2 + 2*n_n2) )

         if (mod(istep,nsamp) == 0) then
            write(*,*) istep,tp(2) - tp(1),tp(3) - tp(2),tp(3) - tp(0)
            write(17,'(8g18.8)') time, EK, energy, Ek + energy ! + repulenergy*0.5_wp
            write(18,'(8g18.8)') time, 2*EK/((3*(2*n_o2 + 2*n_n2 + 3*n_CO2) - 3)*K_ev)
...
!        vxyz = vxyz*sqrt(T(1)/T(2))
      ...
      if ((mod(istep,nsamp) == 0) .AND. (istep >= nequib)) then
         ...
         call msd_samp(n_CO2,rcm_O2_uc,rcm_N2_uc,rcm_CO2_uc)
         call vcf_samp(n_CO2,vcm_O2,vcm_N2,vcm_CO2)
         Ek = 0.0_wp
         EkO2 = 0.0_wp
         EkN2 = 0.0_wp
         EkCO2 = 0.0_wp
         ...
         write(18,'(8g18.8)') time, 2*EK/((3*natom - 3)*K_ev), 2*EkO2/((3*(2*n_o2))*K_ev),&
                                    2*Ekn2/((3*(2*n_n2))*K_ev),2*Ekco2/((3*(3*n_CO2))*K_ev)
         call flush(17);call flush(18)
         write(*,*) istep,tp(2) - tp(1),tp(3) - tp(2),tp(3) - tp(0)
         if (make_movie) then
            iframe = iframe + 1
...
!===================================================== end main loop
      ...
      call  msd_print(n_CO2,nsamp,dtreal)
      call  vcf_print(n_CO2,nsamp,dtreal)
      do i = 1,10
         print *,i,sumtp(i)
      end do

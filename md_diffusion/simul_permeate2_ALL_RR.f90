...
PROGRAM SIMBOX
      ...
      USE vcf_mod
      implicit none
      integer,parameter:: number_moves = 5000
      real(wp),parameter:: dtreal = 4.0_wp
      ...
      real(wp):: r3(3),dt,qj,sumqj,T_kelvin,T_kinetic
      real(wp),allocatable:: rxyzc(:,:),mol_vec(:,:)
      integer,allocatable:: atomc(:)
      real(wp):: CL,time,Ek,Ek_O2,Ek_N2,Ek_CO2,Ek_tot
      real(wp):: rad_Ox(3),rad_N2(3),rad_CO2(3),rr(3),ULJEL,dt_short
      real(wp),allocatable:: massi(:,:)
      real(wp),allocatable:: Ox_massi(:,:,:)
      real(wp),allocatable:: N2_massi(:,:,:)
      real(wp),allocatable:: CO2_massi(:,:,:)
...
!
      ...
      allocate(Ox_fxyz(1:n_O2,1:nat,1:3),N2_fxyz(1:n_N2,1:nat,1:3),CO2_fxyz(1:n_CO2,1:3,1:3))
      allocate(Ox_vxyz(1:n_O2,1:nat,1:3),N2_vxyz(1:n_N2,1:nat,1:3),CO2_vxyz(1:n_CO2,1:3,1:3))
      allocate(Ox_atom(1:n_O2,3),N2_atom(1:n_N2,3),CO2_atom(1:n_CO2,3))
      allocate(Ox_gas_charge(n_O2,3),N2_gas_charge(n_N2,3),CO2_gas_charge(n_CO2,3))
      allocate(Ox_massi(1:n_O2,nat,3),N2_massi(1:n_N2,nat,3),CO2_massi(1:n_CO2,3,3))
...
!
      ...
      if (istep <= nequib) then
         ...
         Ek_tot = Ek + Ek_O2 + Ek_N2 + Ek_CO2

         Ndf = 3*(natom + 2*n_o2 + 2*n_n2 + 3*n_co2) - 3
         T_kinetic = 2.0_wp*EK_tot/(K_ev*Ndf)

         if (mod(istep,nsamp) == 0) then
            write(*,*) istep,tp(2) - tp(1),tp(3) - tp(2),tp(3) - tp(0)
            write(17,'(8g18.8)') time, EK_tot, energy + repulenergy*0.5_wp, Ek_tot + energy + repulenergy*0.5_wp
            call flush(17)
         end if
      ...
      if (istep > nequib) then
         ...
         write (17,'(8g18.8)') time, Ek_tot + energy + repulenergy*0.5_wp
         call flush(17)
         write(*,*) istep,tp(2) - tp(1),tp(3) - tp(2),tp(3) - tp(0)
      end if
      end if

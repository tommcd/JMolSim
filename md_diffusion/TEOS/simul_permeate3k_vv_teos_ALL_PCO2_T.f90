...
CONTAINS
   ...
   SUBROUTINE ACCESSIBLE_REGIONS_Z1(Lattice,nzL,nzU,Accessible,NumAcc)
      ...
      integer:: ix,iy,iz,lab,Nx,Ny,nacc,inc,ic
      !
      Nx = size(Lattice,1)
      Ny = size(Lattice,2)
...
CONTAINS
   ...
   SUBROUTINE insert_mol_GAS(n_add,n_atom,mol_vec,real_xyz,ntrial,rad1,rlower,rupper,n_mol,fover,self_overlap_check)
      integer,intent(in):: ntrial,n_add,n_atom
      integer,intent(inout):: n_mol
      real(wp),intent(in):: mol_vec(:,:),rad1(n_atom)
      real(wp),intent(in):: rlower(:),rupper(:)
      real(wp),intent(inout):: real_xyz(:,:,:)
...
!
print *,'entering insert_mol_GAS'
!print *,'n_mol = ',n_mol

      mol_loop: do im = 1, n_add
...
!print *,'x y z = ',(/x,y,z/)
  ...
  print*,'self overlap check)B'
  print*,'n_atom==B',n_atom
  print*,'n_mol*n_atom=B',n_mol*n_atom
!print *,'?=zero ??  ',mol_vec1(1:NDIM,1)
! first check if there is a large overlap
  ...
  print*,'self overlap check)'
  print*,'n_atom==',n_atom
  print*,'n_mol*n_atom=',n_mol*n_atom
  !print*,'rad1(n_atom)==',rad1(n_atom)
         ...
         n_mol = n_mol + 1
print *,'mol_vec1(1:NDIM,1)  ',mol_vec1(1:NDIM,1)
         do j = 1,n_atom
            real_xyz(n_mol,j,:) = mol_vec1(:,j)
         end do
print *,'>>>>> real_xyz',real_xyz(1,1,:)
         write(*,*) im,n_mol
         cycle mol_loop
...
print*,'radatm(:)',radatm(:)
   ...
   SUBROUTINE insert_mol_accessible(n_mol,n_atom,mol_vec,real_xyz,ntrial,rad1,iz,Accessible,grid,fover,self_overlap_check)
      USE grid_data_mod
      USE nlist_mod
      ...
      real(wp):: rl(3),ru(3)
      integer:: ix,iy,n_add
print*,'insert_mol_accessiblen_atom====',n_atom
      n_add = n_mol/(grid%nbin(1)*grid%nbin(2))
      n_mol = 0
      ...
      do iy = 1,grid%nbin(2)
         ...
         do ix = 1,grid%nbin(1)
            print *,'>>>>> ',ix,iy,iz
            print *,Accessible(ix,iy,iz)
            if (.NOT.Accessible(ix,iy,iz)) cycle
            rl(1) = grid%rlower(1) + (ix-1)*grid%del(1)
            ru(1) = rl(1) + grid%del(1)
            call insert_mol_GAS(n_add,n_atom,mol_vec,real_xyz,ntrial,rad1,rl,ru,n_mol,fover,self_overlap_check)
         end do
      end do
   END SUBROUTINE
...
! O2
      ...
      probe_O2%q = (/q_O_O2,-q_O_O2/)
print*,'probe_O2%rad =====', probe_O2%rad      
! N2
      call new_probe_mol(probe_N2,3)      
      probe_N2%atom = (/ iN_N2, iN_N2, 0 /)
...
!      probe_N2%q = qi(19)
      probe_N2%q = (/q_n_N2, q_n_N2, -2.0*q_n_N2 /)
print*,'probe_N2%rad =====', probe_N2%rad 
...
print*,'probe_CO2%rad =====', probe_CO2%rad 
      !
...
MODULE resize_mod
   ...
   interface resize
      module procedure resize_r1, resize_r2, resize_r3, resize_r4, resize_r5
      module procedure resize_i1, resize_i2, resize_i3, resize_i4, resize_i5
   end interface

contains

   subroutine resize_r1(a,n1)
      ...
      t = a
      deallocate(a)
      allocate(a(lb1:lb1+n1-1))
      a = t(1:n1)
   end subroutine

   subroutine resize_r2(a,n1,n2)
      real(wp),intent(inout),allocatable:: a(:,:)
      integer,intent(in):: n1,n2
      real(wp):: t(size(a,1),size(a,2))
      ...
      t = a
      deallocate(a)
      allocate(a(lb1:lb1+n1-1,lb2:lb2+n2-1))
      a = t(1:n1,1:n2)
   end subroutine

   subroutine resize_r3(a,n1,n2,n3)
      real(wp),intent(inout),allocatable:: a(:,:,:)
      integer,intent(in):: n1,n2,n3
      real(wp):: t(size(a,1),size(a,2),size(a,3))
      ...
      t = a
      deallocate(a)
      allocate(a(lb1:lb1+n1-1,lb2:lb2+n2-1,lb3:lb3+n3-1))
      a = t(1:n1,1:n2,1:n3)
   end subroutine

   subroutine resize_r4(a,n1,n2,n3,n4)
      real(wp),intent(inout),allocatable:: a(:,:,:,:)
      integer,intent(in):: n1,n2,n3,n4
      real(wp):: t(size(a,1),size(a,2),size(a,3),size(a,4))
      ...
      t = a
      deallocate(a)
      allocate(a(lb1:lb1+n1-1,lb2:lb2+n2-1,lb3:lb3+n3-1,lb4:lb4+n4-1))
      a = t(1:n1,1:n2,1:n3,1:n4)
   end subroutine

   subroutine resize_r5(a,n1,n2,n3,n4,n5)
      real(wp),intent(inout),allocatable:: a(:,:,:,:,:)
      integer,intent(in):: n1,n2,n3,n4,n5
      real(wp):: t(size(a,1),size(a,2),size(a,3),size(a,4),size(a,5))
      ...
      t = a
      deallocate(a)
      allocate(a(lb1:lb1+n1-1,lb2:lb2+n2-1,lb3:lb3+n3-1,lb4:lb4+n4-1,lb5:lb5+n5-1))
      a = t(1:n1,1:n2,1:n3,1:n4,1:n5)
   end subroutine

!integers
   subroutine resize_i1(a,n1)
   ...
   subroutine resize_i2(a,n1,n2)
      integer,intent(inout),allocatable:: a(:,:)
      integer,intent(in):: n1,n2
      integer:: t(size(a,1),size(a,2))
      integer:: lb1,ub1,lb2,ub2
      lb1=lbound(a,1)
      ub1=ubound(a,1)
      lb2=lbound(a,2)
   ...
   subroutine resize_i3(a,n1,n2,n3)
      integer,intent(inout),allocatable:: a(:,:,:)
      integer,intent(in):: n1,n2,n3
      integer:: t(size(a,1),size(a,2),size(a,3))
      integer:: lb1,ub1,lb2,ub2,lb3,ub3
      lb1=lbound(a,1)
      ub1=ubound(a,1)
      lb2=lbound(a,2)
   ...
   subroutine resize_i4(a,n1,n2,n3,n4)
      integer,intent(inout),allocatable:: a(:,:,:,:)
      integer,intent(in):: n1,n2,n3,n4
      integer:: t(size(a,1),size(a,2),size(a,3),size(a,4))
      integer:: lb1,ub1,lb2,ub2,lb3,ub3,lb4,ub4
      lb1=lbound(a,1)
      ub1=ubound(a,1)
      lb2=lbound(a,2)
   ...
   subroutine resize_i5(a,n1,n2,n3,n4,n5)
      integer,intent(inout),allocatable:: a(:,:,:,:,:)
      integer,intent(in):: n1,n2,n3,n4,n5
      integer:: t(size(a,1),size(a,2),size(a,3),size(a,4),size(a,5))
      integer:: lb1,ub1,lb2,ub2,lb3,ub3,lb4,ub4,lb5,ub5
      lb1=lbound(a,1)
      ub1=ubound(a,1)
      lb2=lbound(a,2)
...
!
      ...
      if (narg < 3) then
         write(*,*)'usage :'
         write(*,*) trim(carg),'  control_file  atom_file  T[K] irand_seed {xyz_restart  vel_restart} '
         stop
      end if
!
...
!      mol_vec(1,3) = 0.0_wp
      mol_vec(2,:) = 0.0_wp
      mol_vec(3,:) = 0.0_wp
      call insert_mol_accessible(n_O2,n_atom_o2,mol_vec(:,1:n_atom_o2),Ox_xyz,100,rad_Ox,8,&
                                 Accessible(:,:,1:Nz_top,1),grid,fover,check_self_overlap)
...
!      mol_vec(1,3) = 0.0_wp
      mol_vec(2,:) = 0.0_wp
      mol_vec(3,:) = 0.0_wp
      call insert_mol_accessible(n_N2,n_atom_n2,mol_vec(:,1:n_atom_n2),N2_xyz,100,rad_N2,8,&
                                 Accessible(:,:,1:Nz_top,1),grid,fover,check_self_overlap)
      ...
      mol_vec(2,:) = 0.0_wp
      mol_vec(3,:) = 0.0_wp
      call insert_mol_accessible(n_CO2,n_atom_co2,mol_vec(:,1:n_atom_co2),CO2_xyz,100,rad_CO2,8,&
                                 Accessible(:,:,1:Nz_top,1),grid,fover,check_self_overlap)
      !call insert_mol_GAS(n_CO2,n_atom_co2,mol_vec,CO2_xyz,1000000,rad_CO2,Rbox_min1,Rbox_max1,fover,check_self_overlap)
      print *,'n_co2 = ',n_co2
      end if
      call resize(CO2_xyz,n_CO2,n_atom_co2,NDIM)
      call resize(r_CO2_uc,n_CO2,n_atom_co2,3)
      call resize(dr_CO2,n_CO2,n_atom_co2,3)
      call resize(rcm_CO2_uc,n_CO2,3)
      call resize(vcm_CO2,n_CO2,3)
      call resize(image_CO2_xyz,n_CO2,n_atom_co2,NDIM)
      call resize(CO2_fxyz,n_CO2,n_atom_co2,NDIM)
      call resize(CO2_vxyz,n_CO2,n_atom_co2,NDIM)
      call resize(CO2_atom,n_CO2,n_atom_co2)
      call resize(CO2_gas_charge,n_CO2,n_atom_co2)
      call resize(CO2_mass,n_CO2,n_atom_co2,3)
      call resize(CO2_massi,n_CO2,n_atom_co2,3)
      call resize(ULJEL, max(n_o2,n_n2,n_co2,n_gas,n_h2o) )
      call resize(e_bond_CO2,n_co2)
      call resize(e_angle_CO2,n_co2)

...
!
      ...
      do i = 1,nmol
         write(iu,110)'CO ',(CO2_xyz(i,1,k)*Angstrom,k = 1,3)
         write(iu,110)'OC ',(CO2_xyz(i,2,k)*Angstrom,k = 1,3)
         write(iu,110)'OC ',(CO2_xyz(i,3,k)*Angstrom,k = 1,3)
      end do
      if (present(print_all)) then
         if (print_all) then
         do i = 1,natom
...
! INITIAL VELOCITY FROM MAXWELLIAN DISTRIBUTION
      ...
      do j = 1,3
         ...
         RNZ1 = IAZ1*RIMO1
         RNZ2 = IAZ2*RIMO1
         CO2_vxyz(i,j,1) = sqrt(TEMP*K_ev)*SQRT(-2.0_wp*LOG(RNX1))*COS(PI2*RNX2)
         CO2_vxyz(i,j,1) = sqrt(TEMP*K_ev)*SQRT(-2.0_wp*LOG(RNY1))*COS(PI2*RNY2)
         CO2_vxyz(i,j,1) = sqrt(TEMP*K_ev)*SQRT(-2.0_wp*LOG(RNZ1))*COS(PI2*RNZ2)
      end do
      end do
   END SUBROUTINE
...
MODULE charges_mod
   ...
   implicit none
   real(wp),allocatable:: charge(:),CO2_gas_charge(:,:)
...
MODULE vcf_mod
   ...
   real(wp),allocatable:: rac1CO2_av(:)
...
PROGRAM SIMBOX
      ...
      USE vcf_mod
      implicit none
      integer,parameter:: number_moves = 250000
...
!
      ...
      if (istep <= nequib) then
         ...
         T(2) = Ek*2.0_wp/( 3.0_wp*K_ev*(natom + 5*n_CO2) )
         ...
         if (mod(istep,nsamp) == 0) then
            ...
            write(18,'(8g18.8)') time, 2*EK/((3*(3*n_CO2) - 3)*K_ev)
      ...
      if (istep > nequib) then
         ...
         call msd_samp(n_CO2,rcm_CO2_uc)
         call vcf_samp(n_CO2,vcm_CO2)
         Ek = 0.0_wp
         EkCO2 = 0.0_wp
         do m = 1, natom
            Ek = Ek + (0.5_wp/massi(m,1))*dot_product(vxyz(m,:),vxyz(m,:))
         ...
         write (17,'(8g18.8)') time, Ek,EkCO2, Ek + EkCO2 + energy + repulenergy*0.5_wp
         write(18,'(8g18.8)') time, 2*Ekco2/((3*(3*n_CO2))*K_ev)

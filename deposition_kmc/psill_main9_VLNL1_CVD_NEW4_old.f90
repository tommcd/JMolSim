...
! and the flexible and fixed regions of it
      ...
      dLflexible = -5.0_wp/Angstrom  !+ boxl2(3) !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!         call SET_BOX_SIZE(boxl)
!         print '(a,3f12.6)','    boxl = ',boxl*Angstrom
!         print '(a,3f12.6)','   boxl2 = ',boxl2*Angstrom
!   
!!!        call new_frame_file(imve,'frame',10000,'pdb')
!!!        call write_frame(imve,1,natom)
!!         call new_frame_file(imve,'frame',10000,'xyz')
!!         call write_xyz_file(imve,1,natom,atom,rxyz,boxl,box_corners=cuboid(-boxl2,boxl2), &
...
!      end where
      ...
      if (H2O_present) call voidage_check_lattice( probe_tip3p,Rbox_min, Rbox_top, del_bin, void_crit, n_try_void, Lattice(:,:,:,iH2O) )
...
!                           n_try_henry,fover_henry,rcutH,henryk(:,:,1:Nz_top,iTEOS)
      ...
      non_reacting = .false.
      lb_rxn = abs(dLflexible)/Grid%del(3) 
print *, 'lb_rxn =  ',lb_rxn
...
!
            ...
            rate(4,ix,iy,iz) = (k_2r/k_1f)*henryk(ix,iy,iz,iH2O)*P_H2O*ndenrxn(ix,iy,iz,iOSiO)
...
!
            rate(2,ix,iy,iz) = (k_1r/k_1f)*henryk(ix,iy,iz,iH2O)*P_H2O*ndenrxn(ix,iy,iz,iOSiOH3)
...
!
            k_1f = A_1f*exp(-E_A1f/kBoltzT)
            rate(1,ix,iy,iz) = HenryK(ix,iy,iz,iSiOH4)*P_SiOH4*ndenrxn(ix,iy,iz,iOxygenH)
...
!
!     carry out reaction #ir in bin(ix,iy,iz)
print *,'ir = ',ir
...
!
      ...
      case(3)
         ...
         iOH  =  rxns(ib(1),ib(2),ib(3))%r(3)%atom(1)
         call condensation(ia_OH,iOH)
         write(*,*) 'H2O removal ',rxyz(ia_OH,3)
         write(80,*) kmc_step,rxyz(ia_OH,3)
      case(4)

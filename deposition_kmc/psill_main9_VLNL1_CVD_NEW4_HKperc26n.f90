...
include 'Keating_parameters_mod.f90'
...
include 'check_structure_mod.f90'
include 'teos_atomlst_mod.f90'
include 'NLIST_MOD.f90'
include 'verlet_list_mod.f90'
...
include 'repul_energy_mod.f90'
include 'ran_point_sphere.f90'
include 'insert_mod.f90'
include 'Lj_el_mod.f90'
include 'force_en_el_mod.f90'
include 'bonded_force_gas_mod.f90'
include 'RDF_MOD.f90'
...
!
if (kmc_step > 0) call read_rand_state(21)
      do i = 1,size(atmlst)
...
! calculate the Henry's law constant in ALL cells (Accessible = .TRUE.)
      Accessible = .true.
...
!                           n_try_henry,fover_henry,rcutH,henryk(:,:,1:Nz_top,iTEOS)
      call Henrys_law_grid(probe_tip3p,Accessible(:,:,1:Nz_top,iH2O),grid,Nz_top, &
                           n_try_henry,fover_henry,rcutH,henryk(:,:,1:Nz_top,iH2O))



! Mark the cells with henryk > critical value for each Vapor phase reactant
! (e.g. for both Si(OH)4 or TEOS and H2O)
...
write(777,*)
write(777,*) henryk(:,:,1:Nz_top,iSiOH4)
write(777,*)
!
...
!write(999,*)
!write(999,*) 'HK_SiOH4 = ',HK_SiOH4
!write(999,*) 'rate_1 = ',rate(1,ix,iy,iz)
!flush(999)
...
!write(998,*)
!write(998,*) 'n_oh_pairs = ',n_oh_pairs
            if (iOH /= 0) then
...
!write(998,*) '(k_2f/k_1f) = ',(k_2f/k_1f)
!write(998,*) 'Accessible = ',Accessible(ix,iy,iz,iH2O)
!write(998,*) 'rate_3 = ',rate(3,ix,iy,iz)
!flush(998)

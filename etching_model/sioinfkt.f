...
!   OXYGEN NEIGHBORS TO A GIVEN SILICON ATOM
      ...
      DO 99 I=1,NOXX
      DELX=X1(J)-X2(I)
      IF(DELX.GT.CLMX) DELX=DELX-CLX
      IF(DELX.LE.CLNX) DELX=DELX+CLX
      IF(DABS(DELX).GT.SIO) GO TO 99
      DELY=Y1(J)-Y2(I)
      IF(DELY.GT.CLMY) DELY=DELY-CLY
      IF(DELY.LE.CLNY) DELY=DELY+CLY
      IF(DABS(DELY).GT.SIO) GO TO 99
      SQXY=DELY*DELY+DELX*DELX
      IF(SQXY.GT.SQO) GO TO 99
      DELZ=Z1(J)-Z2(I)
      IF(DELZ.GT.CLMZ) DELZ=DELZ-CLZ
      IF(DELZ.LE.CLNZ) DELZ=DELZ+CLZ
      IF(DABS(DELZ).GT.SIO) GO TO 99
      SQXYZ=DELZ*DELZ+SQXY
      IF(SQXYZ.GT.SQO) GO TO 99
      NEO(J) = NEO(J)+1
      NEIGH(J,NEO(J)) = I
      DIST(J,NEO(J)) = SQXYZ
99    CONTINUE
...
!   SILICON NEIGHBORS TO A GIVEN OXYGEN ATOM
      ...
      IF(DELX.GT.CLMX) DELX=DELX-CLX
      IF(DELX.LE.CLNX) DELX=DELX+CLX
      IF(DABS(DELX).GT.SIO) GO TO 995
      DELY=Y1(J)-Y2(I)
      IF(DELY.GT.CLMY) DELY=DELY-CLY
      IF(DELY.LE.CLNY) DELY=DELY+CLY
      IF(DABS(DELY).GT.SIO) GO TO 995
      SQXY=DELY*DELY+DELX*DELX
      IF(SQXY.GT.SQO) GO TO 995
      DELZ=Z1(J)-Z2(I)
      IF(DELZ.GT.CLMZ) DELZ=DELZ-CLZ
      IF(DELZ.LE.CLNZ) DELZ=DELZ+CLZ
      IF(DABS(DELZ).GT.SIO) GO TO 995
      SQXYZ=DELZ*DELZ+SQXY
      IF(SQXYZ.GT.SQO) GO TO 995
      NES(I) = NES(I)+1
      NEIOO(I,NES(I)) = J
      DISO(I,NES(I)) = SQXYZ
995    CONTINUE
...
! **   INITIAL FLUID PARTICLE POSITIONS
       ...
       IF(SQ.LE.SQSIG(1,4+M)) GO TO 50121
       ENDDO
       ...
       RY = RY-DNINT(RY*CLINV)*CL
       SQ = RX*RX+RY*RY+RZ*RZ
       IF(SQ.LE.SQSIG(3,4+M)) GO TO 50121
       ENDDO
       ENDDO
       N(L) = N(L)+2
       N(L+2) = N(L+2)+1
       IF(N(L).GE.40) GO TO 50123
       GO TO 50122
50121  CONTINUE
       ENDDO
50122  CONTINUE
...
! *********
!
                  IF (SQ.GT.SQCUT(L,NF+M)) GO TO 34780
                     RR1 = DSQRT(SQ)
                     RR2 = SQSIG(L,NF+M) / SQ
                     RR6 = RR2 * RR2 * RR2
...
! **   CHECK ON POSSIBLE LIST UPDATE
       ...
       DDY = Y0(L,I)-Y0O(L,I)
       DDZ = Z0(L,I)-Z0O(L,I)
               IF (DDX.GT.CLP) DDX = DDX - CL
               IF (DDX.LE.CLN) DDX = DDX + CL
               IF (DDY.GT.CLP) DDY = DDY - CL
               IF (DDY.LE.CLN) DDY = DDY + CL
               IF (DDZ.GT.CLP) DDZ = DDZ - CL
               IF (DDZ.LE.CLN) DDZ = DDZ + CL
       DDSQ = DDX*DDX+DDY*DDY+DDZ*DDZ
       DMAX = MAX(DDSQ,DMAX)
7708   CONTINUE
       ENDDO
       ENDDO
       IF(DMAX.GT.RLCMIN) THEN
       KKPPI = KKPPI+1
       NEWLIST = 1
       ENDIF
! ***********************************
       IF((ILONG.EQ.1).AND.(ITSTEP.EQ.1)) GO TO 20111
       IF((ITSTEP.EQ.MXSHRT).OR.(NEWLIST.EQ.1)) GO TO 20111
       GO TO 20222
20111  CONTINUE
          NT = 0
          DO L=1,NF
...
!
            IF(J.LE.NT)THEN
                  ...
                  YIJ = Y - YY
                  ZIJ = Z - ZZ
               IF (XIJ.GT.CLP) XIJ = XIJ - CL
               IF (XIJ.LE.CLN) XIJ = XIJ + CL
               IF (YIJ.GT.CLP) YIJ = YIJ - CL
               IF (YIJ.LE.CLN) YIJ = YIJ + CL
               IF (ZIJ.GT.CLP) ZIJ = ZIJ - CL
               IF (ZIJ.LE.CLN) ZIJ = ZIJ + CL
...
!
           ...
           NTE2 = 0
           DO K=1,NFNS
           IF(K.LE.NF) NTE2 = NTE2+N(K)
           IF(K.GT.NF) NTE2 = NTE2+NO(K-NF)
           IF(I.LE.NTE1.AND.J.LE.NTE2) THEN
           RLISTC = RLISTCUT(L,K)
           GO TO 88999
           ENDIF
           ENDDO
...
!
       ...
       ZNEW(L,I) = 2.0*BUFFZN(L,I) - BUFFZO(L,I) + (DLTSQ * AZ(L,I))
          ...
          IF(ILONG.EQ.IEQ) THEN
               BUFFX(L,I) = 0.0
               BUFFY(L,I) = 0.0
               BUFFZ(L,I) = 0.0
           ENDIF
           IF(ILONG.GE.IEQ) THEN
...
! **  THERMAL DIFFUSE SCATTERING FROM THE OXYGEN ATOMS
      ...
      DO 88192 L=1,NF
        ...
        IHIGH = N(L)+NTE
        NTE = NTE+N(L)
       IF(N(L).EQ.0) GO TO 88192
      DO 88193 J=ILOW,IHIGH
                JBEG=NABOR(J)
                JEND=NABOR(J+1)-1
      IF (JBEG.GT.JEND) GO TO 88193
       J1 = J-NNTE(L)
          XXNEW = BUFFXN(L,J1)
          YYNEW = BUFFYN(L,J1)
          ZZNEW = BUFFZN(L,J1)
...
84911  CONTINUE
      ...
      SQ = RX*RX+RY*RY+RZ*RZ
                RO = SQ
       IF(RO.GT.RWMAX(L,NSCOMP)) GO TO 84010
              RZN = ZI0-ZSNEW
              ...
              SQN = RXN*RXN+RYN*RYN+RZN*RZN
                RNN = SQN
           RWAA = RWMIN(L,NSCOMP)
        IF(RO.LT.RWAA.AND.RNN.GT.RWAA) GO TO 48765
        IF(RO.GT.RWAA.AND.RNN.LT.RWAA) GO TO 48765
                GO TO 84010
48765  CONTINUE
...
! **** NEWTON METHOD
        ...
        FMULT = DFNEWT*RELR
        IF(FDELT.GT.FFDLT.OR.FDELT.LT.0.0.OR.FMULT.GT.0.0) THEN
        ...
        IF(ERR.LT.TOL) THEN
        FDELT = FDELT+ERR
        IF(FDELT.GT.FFDLT) FDELT = FFDLT
               GO TO 78398
        ENDIF
78399  CONTINUE
         FDELT = FFDLT
78398  CONTINUE
        IF(FDELT.LT.0.0.OR.FDELT.GT.FFDLT) THEN
...
84710  CONTINUE
        COLLT(NSIL) = FDELT
84010  CONTINUE
         IF(NSIL.EQ.0) GO TO 88193
! ************** FIND SHORTEST COLLISION TIME
         TCOL = 1000.0
         DO 79911 JT=1,NSIL
         IF(COLLT(JT).LT.TCOL) THEN
         TCOL = COLLT(JT)
         ICOL = JT
         ENDIF
79911  CONTINUE
...
!          WRITE(14,*) VEL,VVE
        ...
        RIS = DSQRT(ZIS*ZIS+RRIS)
         ...
         COSP = DABS(YIS/RRIS)
         PHI = DACOS(COSP)
        IF(YIS.GT.0.0.AND.XIS.GT.0.0) PHI = PHI
        IF(YIS.LE.0.0.AND.XIS.GT.0.0) PHI = PI-PHI
        IF(YIS.LE.0.0.AND.XIS.LE.0.0) PHI = PHI+PI
        IF(YIS.GT.0.0.AND.XIS.LE.0.0) PHI = PI2-PHI
        COSP = DCOS(PHI)
        SINT = DSIN(THET)
        SINP = DSIN(PHI)
...
! ********************
      ...
      DO 59192 L=1,NF
        ...
        IHIGH = N(L)+NTE
        NTE = NTE+N(L)
       IF(N(L).EQ.0) GO TO 59192
      DO 59193 J=ILOW,IHIGH
                JBEG=NABOR(J)
                JEND=NABOR(J+1)-1
      IF (JBEG.GT.JEND) GO TO 59193
         J1 = J-NNTE(L)
      DO 34010 JNAB=JBEG,JEND
                 II=LIST(JNAB)
       IF (II.LE.NT) GO TO 34010
                 II1=II-NT
                 JJ = II1
                 DO M=1,NS
                 JJ = JJ-NO(M)
...
!
                 NMM = NM+NF
                     IF (SQ.GT.SQCUT(L,NMM)) GO TO 34010
                        RR1 = DSQRT(SQ)
                        RR2 = SQSIG(L,NMM) / SQ
                        RR6 = RR2 * RR2 * RR2
...
! **  Store each position at interval NSAVE0
          ISAVE0=ISAVE0+1
          IF(ISAVE0.LT.NSAVE0) GO TO 55001
          ISAVE0=0
          JSAVE0=JSAVE0+1
          NPV0=NPV0+1
          IF(NPV0.GT.NSM) NPV0=NPV0-NSM
          DO L=1,2
          K = 0
          DO 51000 I=1,N(L),2
          K = K+1
...
! **  Instantaneous mean square displacement
          IF(JSAVE0.LE.NSM) GO TO 55001
          DO 53000 J=2,NSM
            NN=NPV0-J+1
            IF(NN.LE.0) NN=NN+NSM
            DO L=1,2
            DO 52000 I=1,NNM(L)
...
! *****************************************************************
      ...
      ENDDO
      DO J=1,NSI
      IF(X1(J).GT.CLMX) X1(J)=X1(J)-CLX
      IF(X1(J).LE.CLNX) X1(J)=X1(J)+CLX
      IF(Y1(J).GT.CLMY) Y1(J)=Y1(J)-CLY
      IF(Y1(J).LE.CLNY) Y1(J)=Y1(J)+CLY
      ENDDO
      DO J=1,NOXX
      IF(X2(J).GT.CLMX) X2(J)=X2(J)-CLX
      IF(X2(J).LE.CLNX) X2(J)=X2(J)+CLX
      IF(Y2(J).GT.CLMY) Y2(J)=Y2(J)-CLY
      IF(Y2(J).LE.CLNY) Y2(J)=Y2(J)+CLY
      ENDDO
      NS = 0
! ***********************************
      NSILON = 0
...
558   CONTINUE
      ...
      IC = 0
      DO 10010 J=IP+1,NN
      IF(KOX(J).EQ.IID) THEN
      IC = IC+1
      IMI(IC) = J
      ENDIF
10010   CONTINUE
      IF(IC.EQ.0) GO TO 559
      DO 10110 IL=1,IC
      KOX(IMI(IL)) = KOX(NN)
      NN = NN-1
10110 CONTINUE
      IF(NN.EQ.IP) GO TO 7776
559    CONTINUE
7776   CONTINUE
       NOX = NN
      DO 444 I=1,NOX
...
!   OXYGEN NEIGHBORS TO A GIVEN SILICON ATOM
      ...
      IF(DELX.GT.CLMX) DELX=DELX-CLX
      IF(DELX.LE.CLNX) DELX=DELX+CLX
      IF(DABS(DELX).GT.SIO) GO TO 909
      DELY=Y01(J)-Y02(I)
      IF(DELY.GT.CLMY) DELY=DELY-CLY
      IF(DELY.LE.CLNY) DELY=DELY+CLY
      IF(DABS(DELY).GT.SIO) GO TO 909
      SQXY=DELY*DELY+DELX*DELX
      IF(SQXY.GT.SQO) GO TO 909
      DELZ=Z01(J)-Z02(I)
      IF(DELZ.GT.CLMZ) DELZ=DELZ-CLZ
      IF(DELZ.LE.CLNZ) DELZ=DELZ+CLZ
      IF(DABS(DELZ).GT.SIO) GO TO 909
      SQXYZ=DELZ*DELZ+SQXY
      IF(SQXYZ.GT.SQO) GO TO 909
      NEO(J) = NEO(J)+1
      NEIGH(J,NEO(J)) = I
      DIST(J,NEO(J)) = SQXYZ
909    CONTINUE
...
!   SILICON NEIGHBORS TO A GIVEN OXYGEN ATOM
      ...
      IF(DELX.GT.CLMX) DELX=DELX-CLX
      IF(DELX.LE.CLNX) DELX=DELX+CLX
      IF(DABS(DELX).GT.SIO) GO TO 9905
      DELY=Y01(J)-Y02(I)
      IF(DELY.GT.CLMY) DELY=DELY-CLY
      IF(DELY.LE.CLNY) DELY=DELY+CLY
      IF(DABS(DELY).GT.SIO) GO TO 9905
      SQXY=DELY*DELY+DELX*DELX
      IF(SQXY.GT.SQO) GO TO 9905
      DELZ=Z01(J)-Z02(I)
      IF(DELZ.GT.CLMZ) DELZ=DELZ-CLZ
      IF(DELZ.LE.CLNZ) DELZ=DELZ+CLZ
      IF(DABS(DELZ).GT.SIO) GO TO 9905
      SQXYZ=DELZ*DELZ+SQXY
      IF(SQXYZ.GT.SQO) GO TO 9905
      NES(I) = NES(I)+1
      NEIOO(I,NES(I)) = J
      DISO(I,NES(I)) = SQXYZ
9905    CONTINUE

...
PROGRAM SIMBOX
      ...
      integer,parameter:: number_moves = 200000
...
print *,'dt*unit of time = ',dt*time_s
      ...
      allocate(k1_rk(natom_max,3),k2_rk(natom_max,3))
...
!            call repul_energy(1,natom)
            ...
            call rk_a(dt,massi,rxyz,vxyz,fxyz)
            forall(i=1:natom) rxyz(i,1:3) = rxyz(i,1:3)-boxl*anint(rxyz(i,1:3)*boxli)
...
!            call repul_energy(1,natom)
            ...
            call rk_b(dt,massi,rxyz,coprxyz,vxyz,fxyz)         
...
!               print *,sum(fxyz(1:natom,:),dim=1)
            ...
            if (make_movie.and.mod(istep,200)==0) then

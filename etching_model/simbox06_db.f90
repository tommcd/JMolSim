...
PROGRAM SIMBOX
      ...
      integer:: itmp,ibx,icx,iby,icy,ibz,icz,l,natcl,lst4(4),ipair(2,2),np,ipairs(4)
      integer:: ip(3,4),itry(3),ntry,ia,i1,i2,a1,a2
...
STOP 'ERROR IN CONNECTIVITY'
         ...
         ii = lst4(i)
         if (np == 1) then
            if (ii == ipair(1,2)) cycle outer_pair
         end if
         inner_pair: do j = i + 1,4
            jj = lst4(j)
            if (np == 1) then
               if (jj == ipair(1,1).or.jj == ipair(1,2)) cycle inner_pair
            end if
            if (OH_groups_OK(ii,jj)) then
               np = np + 1
               ipair(np,:) = (/ ii,jj /)
               if (np == 2) exit outer_pair
               exit inner_pair
            end if
         end do inner_pair
         end do outer_pair
...
print *,'condensing ',ipair(1,:)
print *,'proximity ',ipair(1,1)
print *,proximity(ipair(1,1),:)
print *,'proximity ',ipair(1,2)
print *,proximity(ipair(1,2),:)
         call condensation(ipair(1,1),ipair(1,2))
print *
print *,'condensing ',ipair(2,:)
print *,'proximity ',ipair(2,1)
print *,proximity(ipair(2,1),:)
print *,'proximity ',ipair(2,2)
print *,proximity(ipair(2,2),:)
print *
         call condensation(ipair(2,1),ipair(2,2))

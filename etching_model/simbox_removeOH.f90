...
!
      ...
      if (narg < 3) then
         write(*,*)'usage :'
         write(*,*) trim(ctmp),'  in_file natom_max out_file'
         stop 'error: incorrect no. of command line arguments'
      end if
      call get_com_arg(1, infile, "input pdb file")
      call get_com_arg(2, natom_max, "natom_max")
      call get_com_arg(3, outfile, "base name of outputfile")
!
      open(unit=15,file=trim(adjustl(infile)))
...
! Make array of OH atoms
      lstOH%n = 0
      do i = 1,natom
         if (atom(i) == iOxygenH) call add_to_list(lstOH,i)
      end do
      print *,"no. of OH's = ",lstOH%n
      print *,count(atom(1:natom) == iOxygenH)
!
! Perform the etching procedure
!
      isil = 0
      etching: do
         isil = isil + 1
         ! pick an OH to remove
!        call rand_from_list(lstOH,iat)
         do iat = natom,1, -1
            if (atom(iat) == iOxygenH) exit
         end do
         if (iat < 1) exit
...
! remove any dangling Silicons
         do iat = natom,1, -1
            if (atom(iat) /= iSilicon) cycle
            if (count(proximity(iat,:) > 0) >= 2) cycle
!            do j = 1,ncmax(proximity(iat,:))
!               jj = proximity(iat,j)
!               if (jj==0) cycle
!               call delete_atom(jj)
!            end do
             call delete_atom(iat)
             print *,'removed dingling Si ',iat
         end do
...
!         !print *,'n_cluster = ',n_cluster
!         if (n_cluster > 1) then
!!           call remove_from_list(lstOH,iat)
!!           if (atom(iat) ==iSilicon) then
!!              where(lstOH%i == (natom+1)) lstOH%i = iat
!!           end if
!            call restore_config()
!            cycle etching
!         end if
...
!      end do
      ...
      if (spanning_cluster) then
         ...
         open(unit=14,file='struct'//'_'//trim(c6)//'_'//'.pdb')
...
! calculate the density
      ...
      imve = 77
      write(unit=c6,fmt='(i6.6)') natom
      open(unit=imve,file=trim(outfile)//'_'//trim(c6)//'_'//'.pdb')

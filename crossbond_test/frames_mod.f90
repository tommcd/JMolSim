...
CONTAINS
   ...
   SUBROUTINE write_frame(iu,ifirst,ilast)
      ...
      character(len= 132):: frame_file,con_file,xyz_file
      do i = ifirst,ilast
      ...
      frame_file = frame_file(1:j-3)//'scr'  ;print *,'frame_file = ',trim(frame_file)
      con_file = frame_file(1:j-3)//'con'  ;print *,'con_file = ',trim(con_file)
      xyz_file = frame_file(1:j-3)//'xyz'  ;print *,'xyz_file = ',trim(xyz_file)
      call get_free_file_unit(iu2)
      open(unit= iu2,file= trim(xyz_file))
      write(iu2,*)ilast-ifirst+1
      write(iu2,*) boxl
      do i = ifirst,ilast
         write(iu2,'(a,3(1x,f12.6))') atom_name(atom(i)),rxyz(i,1:3)*angstrom
      end do
      close(iu2)
      ...
      do i = 1,nbondtot
         ia = ibond(1,i)
         ib = ibond(2,i)
         if (ANY( ABS(rxyz(ia,1:3)-rxyz(ib,1:3)) > 7.0_wp/angstrom )) then
            fix_bonds = .true.
...
!         write(iu,"(a6,5i5)")'CONECT',ibond(1,i),ibond(2,i)
      ...
      call get_free_file_unit(iu2)
      open(unit= iu2,file= trim(con_file))
      write(iu2,*)ilast-ifirst+1
      write(iu2,*) 'conectivity'
      do i = 1,ilast
         if ( all(proximity(i,:) == 0) ) cycle
         write(iu2,"(5(1x,i6))") i,(proximity(i,j),j = 1,ncmax(atom(i)))
      end do
      close(iu2,status='keep')

      write(iu,110)'HETATM',99991,'AR','   ',99991,-boxl2(1)*angstrom,-boxl2(2)*angstrom,-boxl2(3)*angstrom
      write(iu,110)'HETATM',99992,'AR','   ',99992,-boxl2(1)*angstrom, boxl2(2)*angstrom,-boxl2(3)*angstrom
      write(iu,110)'HETATM',99993,'AR','   ',99993, boxl2(1)*angstrom,-boxl2(2)*angstrom,-boxl2(3)*angstrom
      write(iu,110)'HETATM',99994,'AR','   ',99994, boxl2(1)*angstrom, boxl2(2)*angstrom,-boxl2(3)*angstrom
      write(iu,110)'HETATM',99995,'AR','   ',99995,-boxl2(1)*angstrom,-boxl2(2)*angstrom,boxl2(3)*angstrom
      write(iu,110)'HETATM',99996,'AR','   ',99996,-boxl2(1)*angstrom, boxl2(2)*angstrom,boxl2(3)*angstrom
      write(iu,110)'HETATM',99997,'AR','   ',99997, boxl2(1)*angstrom,-boxl2(2)*angstrom,boxl2(3)*angstrom
      write(iu,110)'HETATM',99998,'AR','   ',99998, boxl2(1)*angstrom, boxl2(2)*angstrom,boxl2(3)*angstrom

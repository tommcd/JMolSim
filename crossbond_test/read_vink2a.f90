...
!
      ...
      open(unit=iu,file='300k.sio2.md')
...
if(ALL(proximity(j,:)/=0)) GOTO 100
               dr = r3-rxyz(j,1:3)
               CALL pbc(dr)
               if (dot_product(dr,dr) < (10.0*bondl_SiO)**2) then
                  nsil=nsil+1
...
100            CONTINUE
         ...
         ia2 = minloc(bond_lens, (bond_lens > bond_lens(ia1(1))))
         j1 = silst(ia1(1))
         j2 = silst(ia2(1))
...
print *,'prox j2  = ',proximity(j2,:)
         ...
         call set_proximity(iox,0,j1)
         call set_proximity(j1,0,iox)
         call set_proximity(iox,0,j2)
         call set_proximity(j2,0,iox)
!         if (nsil > 2) then 
!            print*, 'defect zone,nsil = ', nsil
!         end if
      end do oxygen_loop
      ...
      CALL get_free_file_unit(iu)
      CALL new_frame_file(iu,'300k_SiO2_md',0)

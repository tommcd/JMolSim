...
! O2
      write(*,*) 'O2'
      write(*,*) probe_O2%n
      write(*,'(9i6)') probe_O2%atom
      write(*,'(9f12.6)') probe_O2%rad
      write(*,'(9f12.6)') probe_O2%q
      write(*,'(3f12.6)') probe_O2%r
! N2
      write(*,*) 'N2'
      write(*,*) probe_N2%n
      write(*,'(9i6)') probe_N2%atom
      write(*,'(9f12.6)') probe_N2%rad
      write(*,'(9f12.6)') probe_N2%q
      write(*,'(3f12.6)') probe_N2%r
! CO2
      write(*,*) 'CO2'
      write(*,*) probe_CO2%n
      write(*,'(9i6)') probe_CO2%atom
      write(*,'(9f12.6)') probe_CO2%rad
      write(*,'(9f12.6)') probe_CO2%q
...
!        write(*,'(2i6,4g18.9)')i,j,aij(i,j),bij(i,j),uljcut(i,j),uljsf(i,j)
   ...
   PURE SUBROUTINE ENERGY_LJ_EL2(pr,nl,Ulj,Uel)
      ...
      real(wp):: dr(3),r2,r1,rr6,dele,qL
      integer:: L,M,iaL
      integer:: jj,ic,nc,ncell(343),neigh
      Ulj = 0.0_wp
      Uel = 0.0_wp
      outer_atom_loop: do L = 1,pr%n
      iaL = pr%atom(L)
...
!!>include 'Henrys_law_calc2_mod.f90'
...
! first check if there is a large overlap
         ...
         call ENERGY_LJ_EL2(pr1,3,dUlj,dUel)
...
!
      call henry_profile(probe_N2,dlayer,1)
      open(unit=20,file='khenry_N2.out')
      open(unit=30,file='energy_N2.out')
      call write_file(1,20,30)
!
      call henry_profile(probe_O2,dlayer,2)
      open(unit=21,file='khenry_O2.out')
      open(unit=31,file='energy_O2.out')
...
contains
   ...
   subroutine write_file(ip,iu1,iu2)
      integer,intent(in):: ip,iu1,iu2
      integer:: k,j,i
      real(wp):: x,y,z
      do k = 1,ncelz
         z = (k-0.5_wp)*delz
         do j = 1,ncely
            ...
            do i = 1,ncelx
               x = (i-0.5_wp)*delx
               write(iu1,'(3f9.6,g16.8)') x,y,z,henryk(i,j,k,ip)
               write(iu2,'(3f9.6,g16.8)') x,y,z,Eljel(i,j,k,ip)
            end do
         end do
      end do
      write(20,'(/)')

...
! of the endp oint values). This fully portable, scalar generator has the
...
! The period of this generator is about 3*10**18
      ...
      ix = ieor(ix,ishft(ix,-17))            ! with period 2**32-1
      ix = ieor(ix,ishft(ix,5))
      k = iy/iq                   ! park-miller sequence by schrage's method
      iy = ia*(iy-k*iq)-ir*k      ! period2**31-1
      if(iy < 0)iy = iy+im
      ran = am*ior(iand(im,ieor(ix,iy)),1)       ! combine the 2 generators with masking
end function ran                                 ! to ensure non-zero return value

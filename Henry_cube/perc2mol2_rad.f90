...
PROGRAM perc2mol
      ...
      integer:: ip,im,jp,jm,kp,km,na,cc,ic,ia,nclust
      real(wp):: x,y,z,ravg(3),rmean,rsum,rtmp,r2,rmin2
      real(wp):: henryk(N,N,N)
      real(wp):: rxyz(nbin,3),hk(nbin),rcl(nbin,3)
      integer:: a(N,N,N) = full
      integer:: b(N,N,N) = 0,c(N,N,N) = 0
      integer:: nconn(nbin)=0,proximity(nbin,6)=0
...
!
      nclust = count(nc(1:n_cluster) > 1)
      rsum = 0.0_wp
      print *,'number of clusters with more than 1 element: ',nclust
      do ic = 1, n_cluster
         ravg = 0.0_wp
         ! store the cluster vectors in rcl
         rcl = 0.0_wp
         do ia = 1,nc(ic)
            j = cluster(ic,ia)
print *,rxyz(j,:)
         ...
         ravg = ravg/nc(ic)
print *,'ravg = ',ravg
         ! pick a pivot atom which is close to the centre of the cluster
         ip = 1
         rmin2 = 1.0e20_wp
         do ia = 1,nc(ic)
            r2 = dot_product(rcl(ia,:)-ravg,rcl(ia,:)-ravg)
            if (r2 < rmin2) then
               ip = ia
               rmin2 = r2
            end if
...
print *,'ip = ',ip
         ! now when calculating the radius of gyration of a cluster
         ! we try to keep together clusters which cross the periodic boundaries
         ! by taking distance with respect to the minimum image distance from
         ! the pivot atom
         ravg = 0.0_wp
         do ia = 1,nc(ic)
            rcl(ia,:) = rcl(ia,:) - rcl(ip,:)
print *,rcl(ia,:)
            call pbc(rcl(ia,:))
print *,rcl(ia,:)
            ravg = ravg + rcl(ia,:)
...
print *,'ravg = ',ravg
         rtmp = 0.0_wp
         do ia = 1,nc(ic)
            rtmp = rtmp + dot_product((rcl(ia,:) - ravg),(rcl(ia,:) - ravg))
         end do
         rmean = sqrt(rtmp/nc(ic))
         print *,ic,rmean
         if (nc(ic)==1) cycle
         rsum = rsum + rmean
      end do
...
!
      ...
      do i = 1,na
         ...
         write(iu,"(a6,7i5)")'CONECT',i,(proximity(i,j),j = 1,nconn(i))
      end do

...
! remove z periodicity
      ...
      do i = 1,N
         cc = c(i,j,1)
         if (cc == 0) cycle
         if (.not.in_list(lstc,nlst,cc)) then
            nlst = nlst + 1
            lstc(nlst) = cc
         end if
      end do         
...
!         do jj = 1,N
!         do ii = 1,N
!            if (c(ii,jj,N) == 0) cycle
!            if (c(i,j,1) == c(ii,jj,N) then
!               print *,'z spanning cluster found'
!               print *, c(i,j,1)
!            end if
!         end do
!         end do
...
contains
   ...
   pure function in_list(Lst,N,cc)
      logical:: in_list
      integer,intent(in):: Lst(:),N,cc
      integer:: k
      in_list = .false.
      do k = 1,nlst
         if (cc == lstc(k)) then

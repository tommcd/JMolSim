...
PROGRAM perc2mol
      ...
      real(wp):: Kcrit = 0.035_wp,Kbegin,Kend,delK
      integer:: i,j,k,narg,len,status,nb,iu
      integer:: ip,im,jp,jm,kp,km,na,cc,itry
      real(wp):: x,y,z
...
!
      ...
      if(narg < 4)then
         write(*,*)'usage :'
         write(*,*) ctmp(1:len),' Kbegin Kend infile outfile'
         stop
      end if
!
      call get_command_argument (1, ctmp, len, status)
      if (status /= 0) then
         write (*,*) 'get_command_argument failed: status = ', status, ' arg = ', 1
         stop
      end if
      write (*,*) 'arg = ', ctmp(1:len)
      read(unit=ctmp,fmt=*) Kbegin ; print *,'Kbegin = ',Kbegin
...
!
      call get_command_argument (3, infile, len, status)
      if (status /= 0) then
         write (*,*) 'get_command_argument failed: status = ', status, ' arg = ', 3
         stop
      end if
      write (*,*) 'arg = ', infile(1:len)
      open(unit=14,file=trim(infile(1:len)))
!
      call get_command_argument (4, outfile, len, status)
      if (status /= 0) then
...
!
      ...
      outer_loop: do itry = 1,ntry

      Kcrit = Kbegin + (itry-1)*delK
...
!      print *,'lst_top: ',lst_top(1:ntop)
      ...
      print *,itry,' there are ',nspan,' spanning clusters in the z-direction:'
      do i = 1,nspan
         print *,lst_span(i)
      end do

      if (nspan == 0) exit outer_loop
